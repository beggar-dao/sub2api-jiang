      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <div class="card">
            ...
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ localText("邮箱快捷登录", "Email OAuth Login") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{
                  localText(
                    "开启 GitHub 或 Google 邮箱授权登录后，系统会读取已验证邮箱，存在则直接登录，不存在则自动注册。",
                    "Once GitHub or Google email OAuth is turned on, the system reads the verified email: existing users are signed in directly, while new users are registered automatically.",
                  )
                }}
              </p>
            </div>
                      ...
                      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {{
                          localText(
                            ...
                            "The GitHub OAuth App requires read:user user:email scopes. Fill in the backend callback URL shown below.",
                          )
                        }}
                      </p>
                    </div>
                  ...
                  <div v-if="form.github_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-dark-800 dark:text-gray-300">
                      <template v-if="isZhLocale">
                        配置指引：GitHub Settings → Developer settings →
                        <a
                        ...
                        >OAuth Apps</a>
                        → New OAuth App；Homepage URL 填写站点域名，Authorization callback URL 填写下方后端回调地址。
                      </template>
                      <template v-else>
                        ...
                        <a
                          ...
                          class="font-medium text-primary-600 hover:underline dark:text-primary-400"
                        >OAuth Apps</a>
                        → New OAuth App. Set Homepage URL to your site origin, and Authorization callback URL to the backend callback URL shown below.
                      </template>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                      <div>
                        ...
                        <input
                          ...
                          type="text"
                          class="input font-mono text-sm"
                          placeholder="GitHub OAuth Client Identifier"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          :placeholder="
                            form.github_oauth_client_secret_configured
                              ? localText('密钥已配置，留空保持不变。', 'Secret already configured. Leave blank to keep it.')
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                        {{ localText("后端回调地址", "Server Callback URL") }}
                      </label>
                      <input
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                        {{ localText("前端回跳地址", "Client Callback URL") }}
                      </label>
                      <input
                ...
                <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-700">
                      ...
                      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {{
                          localText(
                            ...
                            "The Google OAuth client requires openid email profile scopes and must have the backend callback URL registered among its credentials.",
                          )
                        }}
                      </p>
                    </div>
                  ...
                  <div v-if="form.google_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-dark-800 dark:text-gray-300">
                      {{
                        localText(
                          "配置指引：Google Cloud Console → APIs & Services → OAuth consent screen 完成同意屏幕；Credentials → Create Credentials → OAuth client ID，类型选择 Web application，并将下方地址加入 Authorized redirect URIs。",
                          "Setup guide: Google Cloud Console → APIs & Services → OAuth consent screen to finish the consent screen; then Credentials → Create Credentials → OAuth client ID, pick Web application, and append the URL below to Authorized redirect URIs.",
                        )
                      }}
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                      <div>
                        ...
                        <input
                          ...
                          type="text"
                          class="input font-mono text-sm"
                          placeholder="Google OAuth Client Identifier"
                        />
                      </div>
                      <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Client Secret</label>
                ...
                <div class="space-y-4">
                  ...
                  >
                    <div class="flex items-start justify-between gap-4">
                      <div>
                        <h3 class="font-medium text-gray-900 dark:text-white">
                          {{ localText("PC 应用", "PC Application") }}
                        </h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                          {{
                            localText(
                              "桌面浏览器通过微信开放平台扫码登录。可与公众号或移动应用同时存在。",
                              "Desktop browsers sign in via the WeChat Open Platform QR code flow. It can coexist with the Official Account or Mobile App.",
                            )
                          }}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          :placeholder="
                            localText(
                              '微信开放平台 PC 应用 AppID',
                              'WeChat Open Platform PC Application ID',
                            )
                          "
                        />
                      </div>
                          ...
                          :placeholder="
                            form.wechat_connect_open_app_secret_configured
                              ...
                              : localText(
                                  ...
                                  'WeChat Open Platform PC Application Secret',
                                )
                          "
                        />
                      </div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                          {{
                            localText(
                              ...
                              "Only accessible inside the WeChat browser. It appears as unavailable in non-WeChat environments.",
                            )
                          }}
                        </p>
                      </div>
                  ...
                  >
                    <div class="flex items-start justify-between gap-4">
                      <div>
                        <h3 class="font-medium text-gray-900 dark:text-white">
                          {{ localText("移动应用", "Mobile Application") }}
                        </h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                          {{
                            localText(
                              "原生移动端通过微信 SDK 唤起授权，网页端不会直接发起该流程。",
                              "Native mobile apps initiate authorization via the WeChat SDK. The web UI does not trigger this flow directly.",
                            )
                          }}
                        </p>
                      </div>
                    ...
                    >
                      <div>
                        <label
                          class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                        >
                          {{ localText("移动应用 AppID", "Mobile Application ID") }}
                        </label>
                        <input
                      ...
                      <div>
                        <label
                          class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300"
                        >
                          {{ localText("移动应用 AppSecret", "Mobile Application Secret") }}
                        </label>
                        <input
                ...
                >
                  {{
                    localText(
                      ...
                      "If the PC App is enabled alongside the Official Account or Mobile App, all of them must belong to the same WeChat Open Platform entity; otherwise UnionID cannot reliably merge identities.",
                    )
                  }}
                </div>
                    ...
                    <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                      {{
                        localText(
                          ...
                          "Serves as the browser callback for the PC App and Official Account. The native mobile SDK flow does not rely on this browser callback.",
                        )
                      }}
                    </p>
                    <div
                    ...
                    <div v-if="form.dingtalk_connect_sync_display_name" class="space-y-2">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <input
                          ...
                          type="text"
                          :placeholder="localText('钉钉姓名', 'DingTalk Display Name')"
                          class="input text-sm flex-1 max-w-xs"
                        />
                      </div>
                    </div>
                    ...
                    <div v-if="form.dingtalk_connect_sync_corp_email" class="space-y-2">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <input
                          ...
                          type="text"
                          :placeholder="localText('钉钉企业邮箱', 'DingTalk Corp. Email')"
                          class="input text-sm flex-1 max-w-xs"
                        />
                      </div>
                    </div>
                    ...
                    <div v-if="form.dingtalk_connect_sync_dept" class="space-y-2">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <input
                          ...
                          type="text"
                          :placeholder="localText('钉钉部门', 'DingTalk Dept.')"
                          class="input text-sm flex-1 max-w-xs"
                        />
                      </div>
                    </div>
	          ...
	          <div class="card">
	              ...
	              <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
	                  ...
	                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
	                    {{
	                      localText(
	                        ...
	                        "Control whether the login page asks users to review and accept Markdown policy documents before proceeding.",
	                      )
	                    }}
	                  </p>
	                </div>
	                <div class="flex items-center gap-3">
	                  <span class="text-sm text-gray-600 dark:text-gray-300">
	                    {{ form.login_agreement_enabled ? localText("已启用", "Active") : localText("未启用", "Inactive") }}
	                  </span>
	            ...
	            <div class="space-y-6 p-6">
                  ...
                  <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {{
                      form.login_agreement_mode === "checkbox"
                        ? localText("复选框会显示在登录按钮下方，未勾选前所有登录入口禁用。", "The checkbox appears beneath the login button and blocks all login entry points until ticked.")
                        : localText("弹窗会在登录页打开，用户拒绝后所有登录入口保持禁用。", "The modal opens on the login page and keeps all login entry points disabled until the user accepts.")
                    }}
                  </p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ localText("日期或文档内容变化后，用户需要重新同意。", "Updating the date or content forces users to accept again.") }}
                  </p>
                </div>
              </div>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      {{
                        localText(
                          ...
                          "Document titles are customizable and the body is stored as Markdown.",
                        )
                      }}
                    </p>
                  </div>
                      ...
                      <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                          <p class="truncate text-sm font-semibold text-gray-900 dark:text-white">
                            {{ doc.title || localText("未命名文档", "Unnamed document") }}
                          </p>
                    ...
                    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                      <div>
                        ...
                        <input
                          ...
                          type="text"
                          class="input text-sm"
                          :placeholder="localText('例如：服务条款', 'e.g. Terms of Service')"
                        />
                      </div>
                      <div>
                        <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
                    ...
                    <div class="mt-3">
                      <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
                        ...
                        <textarea
                          ...
                          rows="8"
                          class="input font-mono text-sm"
                          :placeholder="localText('在这里填写正式 Markdown 内容。', 'Enter the finalized Markdown content here.')"
                        ></textarea>
                    </div>
                  </div>
                </div>
...
function defaultLoginAgreementDocuments(): LoginAgreementDocument[] {
  return [
    ...
    {
      id: "usage-policy",
      title: localText("使用政策", "Acceptable Use Policy"),
      content_md: "",
    },
    {
      id: "supported-regions",
      title: localText("支持的国家和地区", "Supported Countries & Regions"),
      content_md: "",
    },
    {
...
const form = reactive<SettingsForm>({
  ...
  dingtalk_connect_sync_corp_email_attr_name: localText("钉钉企业邮箱", "DingTalk Corp. Email"),
  dingtalk_connect_sync_display_name_attr_name: localText("钉钉姓名", "DingTalk Display Name"),
  dingtalk_connect_sync_dept_attr_name: localText("钉钉部门", "DingTalk Dept."),
...
const authSourceDefaultsMeta = computed(() => [
  ...
  },
  {
    ...
    description: localText(
      ...
      "Takes effect when a user first signs up or links their account via a verified GitHub email.",
    ),
  },
  {
    ...
    description: localText(
      ...
      "Takes effect when a user first signs up or links their account via a verified Google email.",
    ),
  },
  {
    ...
    description: localText(
      "通过钉钉首次注册或首次绑定时应用。",
      "Takes effect when a user first signs up or links their account via DingTalk.",
    ),
  },
]);
...
async function setAndCopyEmailOAuthRedirectUrl(provider: EmailOAuthProvider) {
  ...
  await copyToClipboard(
    url,
    localText("回调地址已写入并复制。", "Callback URL applied and copied."),
  );
}
  ...
  try {
    ...
    if (form.login_agreement_enabled && normalizedLoginAgreementDocuments.length === 0) {
      appStore.showError(
        localText(
          ...
          "At least one document must remain when login agreement is enabled.",
        ),
      );
      return;
    }
    ...
    if (emptyTitleDocument) {
      appStore.showError(
        localText(
          "登录条款文档名称不能为空。",
          "Login agreement document titles must not be empty.",
        ),
      );
      return;
    }
    ...
    if (duplicateLoginAgreementDocumentId) {
      appStore.showError(
        localText(
          ...
          `Duplicate login agreement document route detected: /legal/${duplicateLoginAgreementDocumentId}`,
        ),
      );
      return;
    }
    ...
    if (form.wechat_connect_mp_enabled && form.wechat_connect_mobile_enabled) {
      appStore.showError(
        localText(
          "公众号和移动应用不能同时启用。",
          "Official Account and Mobile App must not be turned on simultaneously.",
        ),
      );
      return;
    }
