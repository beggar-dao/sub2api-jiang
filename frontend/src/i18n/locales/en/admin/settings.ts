export default {
    settings: {
      ...
      features: {
        channelMonitor: {
          ...
          description: 'Periodically probes configured channels and surfaces availability / latency to users. Turning it off stops the scheduler and returns an empty list on the user page.',
        ...
        riskControl: {
          title: 'Risk Control',
          description: 'Enables the content-moderation menu and the gateway audit entry point. Disabled by default.',
          ...
          cyberSessionBlockHint: 'When enabled, sessions hit by an upstream cyber_policy are blocked locally for the TTL and no longer forwarded. Only the offending session is blocked; other sessions on the same key are unaffected.',
        ...
        affiliate: {
          ...
          freezeHoursDesc: 'New rebates stay frozen for this period before they become available for withdrawal. 0 = no freeze.',
          durationDays: 'Rebate Duration (days)',
          durationDaysDesc: 'The rebate relationship expires after this many days since the invitee registered. 0 = permanent.',
          ...
          customUsers: {
            ...
            resetMessage: 'Reset every custom setting for {email}?\n• The exclusive rebate rate will be cleared (falls back to the global rate)\n• The invite code will be regenerated as a new system code (previously shared links will stop working)',
          ...
          modal: {
            ...
            codeHint: '4-32 characters; A-Z, 0-9, underscore, dash. Leave empty to keep the current one. Input is upper-cased.',
            ...
            errorEmpty: 'Fill in at least one: a custom invite code or an exclusive rebate rate',
          },
          batchModal: {
            title: 'Batch Set Rate ({count} users selected)',
            hint: 'Apply the same exclusive rebate rate to every selected user.',
            placeholder: 'e.g. 30',
            clearHint: 'Submitting empty clears the exclusive rate for the selected users.',
          },
        },
      },
      ...
      registration: {
        ...
        totpKeyNotConfigured:
          'Please configure TOTP_ENCRYPTION_KEY in the environment variables first. Generate a key with: openssl rand -hex 32'
      },
      turnstile: {
      ...
      apiKeyAcl: {
        ...
        trustForwardedIpHint:
          'Disabled by default. Enable only when the origin is reachable solely through a Cloudflare or Nginx reverse proxy. When enabled, API Key IP allowlists and denylists use CF-Connecting-IP, X-Real-IP, or X-Forwarded-For, matching the request IP shown in usage records.'
      },
      linuxdo: {
        ...
        clientSecretHint: 'Used by the backend to exchange tokens (keep it secret)',
      ...
      dingtalk: {
        ...
        redirectUrlHint:
          'Must match the redirect URL configured in the DingTalk Open Platform (must be an absolute http(s) URL)',
        corpPolicy: {
        ...
        syncDisplayNameHint: 'Overwrite the username with the DingTalk staff name on each login (also stored in the dingtalk_name attribute).',
        ...
        syncDisplayNameTargetHint: 'Defaults to dingtalk_name / DingTalk Name. Saving settings auto-creates the user attribute by the key and display name above (an existing definition only has its display name synced).',
        syncCorpEmailTarget: 'Attribute key',
        syncCorpEmailTargetHint: 'Defaults to dingtalk_email / DingTalk Corporate Email. Saving settings auto-creates the user attribute by the key and display name above (an existing definition only has its display name synced).',
        syncDeptTarget: 'Attribute key',
        syncDeptTargetHint: 'Defaults to dingtalk_department / DingTalk Department. Saving settings auto-creates the user attribute by the key and display name above (an existing definition only has its display name synced).',
      ...
      oidc: {
        ...
        discoveryUrlPlaceholder: 'Optional — leave empty to auto-derive from the issuer',
        ...
        frontendRedirectUrlHint: 'Frontend route used after the backend callback',
      ...
      defaults: {
        ...
        affiliateRebateRateHint:
          'Rebate percentage credited to the inviter after a recharge (0-100%, e.g. 10 means 10%)',
        defaultConcurrency: 'Default Concurrency',
        defaultConcurrencyHint: 'Maximum concurrent requests for new users',
        defaultUserRpmLimit: 'Default User RPM Limit',
        defaultUserRpmLimitHint: 'Default max requests per minute for new users; 0 = unlimited. Applied only at new-user creation.',
        defaultSubscriptions: 'Default Subscriptions',
        defaultSubscriptionsHint: 'Auto-assign these subscriptions when a new user is created or registers',
        ...
        defaultSubscriptionsDuplicate:
          'Duplicate subscription group: {groupId}. Each group can appear only once.',
        ...
        platformQuotaNotice: 'The monthly quota uses a 30-day rolling window, not a calendar month.',
      },
      platformQuota: {
        platform:    'Platform',
        daily:       'Daily (USD)',
      ...
      claudeCode: {
        ...
        minVersionHint:
          'Reject Claude Code clients below this version (semver format). Leave empty to disable the version check.',
      ...
      scheduling: {
        ...
        allowUngroupedKeyHint: 'When disabled, API Keys not assigned to any group cannot make requests (403 Forbidden). Keep it disabled to ensure every Key belongs to a specific group.'
      },
      gatewayForwarding: {
        ...
        metadataPassthroughHint: 'Pass through the client\'s original metadata.user_id without rewriting. May improve upstream cache hit rates.',
        cchSigning: 'CCH Signing',
        cchSigningHint: 'Sign the billing header in forwarded requests with a CCH hash. When disabled, the placeholder is preserved.',
        ...
        anthropicCacheTTL1hInjectionHint: 'When enabled, existing ephemeral cache_control blocks in Anthropic OAuth/Setup Token request bodies are forced to 1h; response usage is billed back as 5m by default, with the account-level TTL billing override taking priority.',
        ...
        clientDatelineNormalizationHint: 'Default on. Rewrites the "Today\'s date is …" sentence in Anthropic OAuth/Setup Token requests back to a canonical ASCII apostrophe and hyphen date format, erasing the steganographic fingerprint bits some clients inject when they detect a non-official base URL. Applies to system prompts and <system-reminder> blocks only; API-Key accounts are unaffected.',
        ...
        openaiCodexUserAgentHint: 'Used to bypass Cloudflare browser-UA challenges on the OpenAI upstream. Applies only when the client User-Agent is detected as a browser (Mozilla/...). Leave empty to use the built-in default.',
        codexHardeningTitle: "Codex Settings",
        codexClientRestrictionTitle: "Codex client restriction",
        codexHardeningDesc:
          "Only affects OpenAI OAuth accounts with 'Codex official clients only' enabled (global). Beyond User-Agent/Originator, it hardens the decision with a version range, an engine-fingerprint gate, and black/whitelists.",
        ...
        codexVersionHint:
          "Official clients only: their version is checked against the [min, max] range. Leave a side empty to skip that limit.",
        ...
        codexWhitelistDesc:
          "Allow clients outside the official set: requires an exact originator and every User-Agent marker present. Still subject to the fingerprint gate unless 'Skip engine fingerprint' is checked.",
      ...
      webSearchEmulation: {
        ...
        quotaUsage: 'Usage',
        resetUsage: 'Reset',
        resetUsageConfirm: 'Reset the usage counter for this provider?',
      ...
      site: {
        ...
        backendModeDescription:
          'Disables user registration, the public site, and self-service features. Only admin can log in and manage the platform.',
        siteName: 'Site Name',
        siteNamePlaceholder: 'Sub2API',
        siteNameHint: 'Displayed in emails and page titles',
        siteSubtitle: 'Site Subtitle',
        siteSubtitlePlaceholder: 'Subscription to API Conversion Platform',
        siteSubtitleHint: 'Displayed on the login and register pages',
        apiBaseUrl: 'API Base URL',
        apiBaseUrlPlaceholder: 'https://api.example.com',
        apiBaseUrlHint:
          'Used for "Use Key", "Import to CC Switch", and callback URL suggestions. Leave empty to use the current site URL.',
        ...
        customEndpoints: {
          ...
          description: 'Add extra API endpoint URLs for users to quickly copy on the API Keys page',
        ...
        contactInfoHint: 'Customer-support contact info, shown on the redeem page, profile, etc.',
        ...
        remove: 'Remove',
        logoHint: 'PNG, JPG, or SVG. Max 300KB. Recommended: an 80x80px square image.',
        logoSizeError: 'Image size exceeds the 300KB limit ({size}KB)',
        logoTypeError: 'Please select an image file',
        logoReadError: 'Could not read the image file',
        homeContent: 'Home Page Content',
        homeContentPlaceholder: 'Enter custom content for the home page. Supports Markdown & HTML. If a URL is entered, it is shown as an iframe.',
        homeContentHint: 'Customize the home page content. Supports Markdown/HTML. If you enter a URL (starting with http:// or https://), it is used as an iframe src to embed an external page. When set, the default status information is no longer shown.',
        homeContentIframeWarning: '⚠️ iframe mode note: some websites have X-Frame-Options or CSP security policies that prevent embedding in iframes. If the page appears blank or shows an error, please verify the target website allows embedding, or consider using HTML mode to build your own content.',
        hideCcsImportButton: 'Hide CCS Import Button',
        hideCcsImportButtonHint: 'When enabled, the "Import to CCS" button is hidden on the API Keys page'
      },
      purchase: {
        ...
        enabledHint: 'Shown only in standard mode (not simple mode)',
      ...
      soraClient: {
        ...
        enabledHint: 'When enabled, the Sora entry is shown in the sidebar so users can access Sora features'
      },
      customMenu: {
      ...
      payment: {
        ...
        enabledPaymentTypesHint: 'Disabling a provider also disables its instances.',
        findProvider: 'Looking for a suitable EasyPay provider?',
        minAmount: 'Minimum Amount',
        maxAmount: 'Maximum Amount',
        dailyLimit: 'Daily Limit',
        balanceRechargeMultiplier: 'Balance Recharge Multiplier',
        balanceRechargeMultiplierHint: 'How many USD of balance the user receives for each 1 CNY paid',
        ...
        subscriptionUsdToCnyRateHint:
          'CNY charged per 1 USD of plan price on CNY channels (e.g. 7.15). 0 or empty = disabled; the plan price is charged as-is. When enabled, every plan price must be set in USD',
        subscriptionUsdToCnyRateDisabled: 'Disabled (price charged as-is)',
        rechargeFeeRate: 'Recharge Fee Rate',
        rechargeFeeRateHint: 'Percentage of service fee charged on top of the recharge amount; 0 means no fee',
        ...
        helpImagePlaceholder: 'Upload or enter an image URL',
        ...
        validationTypesRequired: 'Please pick at least one supported payment type',
        validationFieldRequired: '{field} is required',
        validationEasyPayCustomMethodRequired: 'Each custom EasyPay method needs both a payment type and an upstream type',
        validationEasyPayCustomMethodTypeInvalid: 'Custom EasyPay payment types may contain only lowercase letters, digits, underscores, and hyphens',
        validationEasyPayCustomMethodUpstreamTypeInvalid: 'EasyPay upstream types may contain only lowercase letters, digits, underscores, and hyphens',
        validationEasyPayCustomMethodReserved: 'Custom EasyPay payment types cannot use the built-in alipay or wxpay',
        ...
        wxpayConfigHint: 'WeChat Pay usually needs only the App ID. Fill MP App ID, H5 App Name, and H5 App URL only when your Official Account or H5 flow specifically requires them.',
        ...
        field_paymentCurrencyHint: 'Defaults to CNY. Stripe and Airwallex can pick HKD, USD, or another listed currency supported by the account; WeChat Pay, Alipay, and EasyPay remain CNY.',
        ...
        easypayCustomMethodsHint: 'Add provider-specific methods supported by this EasyPay endpoint. The payment type is stored on Sub2API orders; the upstream type is sent as the EasyPay type.',
        ...
        stripeWebhookHint: 'Configure the following URL as a Webhook endpoint in the Stripe Dashboard:',
        ...
        airwallexWebhookHint: 'Configure the following URL as a Webhook endpoint in Airwallex. Select at least Payment Intent -> Succeeded (payment_intent.succeeded), preferably also Payment Intent -> Cancelled (payment_intent.cancelled). Use the account default or the latest stable API version.',
        ...
        airwallexGuideNote: 'Do not grant unrelated permissions such as Spend, Payouts, Transfers, Funds Splits, or POS Terminals unless you explicitly need them. For webhooks, select at least payment_intent.succeeded, preferably also payment_intent.cancelled, and use the account default or the latest stable API version.',
        limitsTitle: 'Limits',
        limitSingleMin: 'Min per order',
        limitSingleMax: 'Max per order',
        limitDaily: 'Daily limit',
        limitsHint: 'All empty = use the global config; partially filled = empty means no limit',
        ...
        deleteProviderConfirm: 'Remove this provider?',
        providerName: 'Provider Name',
        providerKey: 'Provider Type',
        selectProviderKey: 'Pick a Provider Type',
        providerConfig: 'Credentials',
        paymentGuideTrigger: 'View payment guide',
        guideOpenLabel: 'Enable: ',
        guideCallLabel: 'Call: ',
        guideFallbackLabel: 'Fallback: ',
        alipayGuideSummary: 'Desktop prefers QR precreate and falls back to the cashier; mobile prefers WAP checkout.',
        ...
        wxpayGuideSummary: 'Desktop prefers Native QR; mobile routes to JSAPI or H5 based on the browser context.',
        ...
        wxpayGuideJsapiOpen: 'Enable Official Account payment and make sure the browser is inside WeChat with an available OpenID.',
      ...
      balanceNotify: {
        ...
        description: 'Send an email notification when the user balance falls below a threshold',
        enabled: 'Enable Balance Low Notification',
        threshold: 'Default Threshold',
        thresholdHint: 'Used when the user has not set a custom value',
        thresholdPlaceholder: 'Enter amount',
        rechargeUrl: 'Recharge Page URL',
        rechargeUrlPlaceholder: 'https://example.com/payment',
        rechargeUrlHint: 'A top-up button appears in the email when set',
      },
      quotaNotify: {
        title: 'Account Quota Notification',
        description: 'Notify admins when account quota usage reaches the alert threshold',
      ...
      smtp: {
        ...
        passwordPlaceholder: '********',
        passwordHint: 'Leave empty to keep the existing password',
        ...
        useTls: 'Use TLS',
        useTlsHint: 'Enable TLS encryption for the SMTP connection'
      },
      testEmail: {
      ...
      emailTemplates: {
        ...
        previewSecurityHint: 'Preview HTML is generated by the backend preview endpoint and shown in a sandboxed iframe with scripts disabled.',
      ...
      opsMonitoring: {
        ...
        queryModeHint: 'Default query mode for the Ops Dashboard (auto/raw/preagg)',
      ...
      adminApiKey: {
        ...
        create: 'Create Key',
        creating: 'Creating...',
        regenerateConfirm: 'Are you sure? The current key will be invalidated immediately.',
        deleteConfirm:
          'Delete the admin API key? External integrations will stop working.',
        keyGenerated: 'New admin API key generated',
        keyDeleted: 'Admin API key removed',
        copyKey: 'Copy Key',
        keyCopied: 'Key copied to clipboard',
        keyWarning: 'This key is shown only once. Please copy it now.',
        securityWarning: 'Warning: this key provides full admin access. Keep it secure.',
        usage: 'Usage: add to the request header — x-api-key: <your-admin-api-key>'
      },
      soraS3: {
        title: 'Sora Storage',
        ...
        selectProvider: 'Pick a Storage Type',
        ...
        profileDeleted: 'Storage profile removed',
        profileActivated: 'Active storage profile switched',
        profileIDRequired: 'Profile ID is required',
        profileNameRequired: 'Profile name is required',
        profileSelectRequired: 'Please pick a profile first',
        endpointRequired: 'An S3 endpoint is required when enabled',
        bucketRequired: 'A bucket is required when enabled',
        accessKeyRequired: 'An Access Key ID is required when enabled',
        ...
        enabledHint: 'When enabled, Sora-generated media files are uploaded automatically',
        ...
        cdnUrl: 'CDN URL',
        cdnUrlHint: 'Optional. When configured, files are accessed via the CDN URL',
        forcePathStyle: 'Force Path Style',
        defaultQuota: 'Default Storage Quota',
        defaultQuotaHint: 'Default quota when not specified at the user or group level. 0 means unlimited',
        testConnection: 'Test Connection',
        testing: 'Testing...',
        ...
        gdrive: {
          ...
          authorizeHint: 'Get a Refresh Token via OAuth2',
          oauthFieldsRequired: 'Please fill in the Client ID and Client Secret first',
      ...
      overloadCooldown: {
        ...
        description: 'Configure the account scheduling-pause strategy when upstream returns 529 (overloaded)',
        enabled: 'Enable Overload Cooldown',
        enabledHint: 'Pause account scheduling on 529 errors; auto-recover after the cooldown',
        cooldownMinutes: 'Cooldown Duration (minutes)',
        cooldownMinutesHint: 'How long to pause account scheduling (1-120 minutes)',
        saved: 'Overload cooldown settings saved',
        saveFailed: 'Could not save overload cooldown settings'
      },
      rateLimit429Cooldown: {
        ...
        enabledHint: 'Pause account scheduling when a 429 has no reset time, then auto-recover after the cooldown',
        ...
        saveFailed: 'Could not save 429 default cooldown settings'
      },
      streamTimeout: {
        title: 'Stream Timeout Handling',
        description: 'Configure the account-handling strategy when the upstream response times out',
        enabled: 'Enable Stream Timeout Handling',
        enabledHint: 'Automatically handle problematic accounts when the upstream times out',
        timeoutSeconds: 'Timeout Threshold (seconds)',
        timeoutSecondsHint: 'A stream data interval exceeding this time counts as a timeout (30-300s)',
        action: 'Action',
        actionTempUnsched: 'Temporarily Unschedulable',
        actionError: 'Mark as Error',
        actionNone: 'No Action',
        actionHint: 'Action to take on the account after a timeout',
        tempUnschedMinutes: 'Pause Duration (minutes)',
        tempUnschedMinutesHint: 'Duration of the temporary unschedulable state (1-60 minutes)',
        thresholdCount: 'Trigger Threshold (count)',
        thresholdCountHint: 'Number of timeouts before triggering the action (1-10)',
        ...
        saveFailed: 'Could not save stream timeout settings'
      },
      rectifier: {
        ...
        enabledHint: 'Master switch — disabling turns off every rectification feature',
        ...
        thinkingBudgetHint: 'Automatically set the budget to 32000 and retry when upstream returns a budget_tokens constraint error (≥1024)',
        ...
        saveFailed: 'Could not save rectifier settings'
      },
      betaPolicy: {
        title: 'Beta Policy',
        description: 'How to handle Beta features when configuring the forwarding of Anthropic API requests. Applies only to the /v1/messages endpoint.',
        action: 'Action',
        ...
        saveFailed: 'Could not save beta policy settings',
        modelWhitelist: 'Model Whitelist',
        modelWhitelistHint: 'Leave empty to apply to every model. Supports exact match and wildcard prefix (e.g., claude-opus-*)',
      ...
      openaiFastPolicy: {
        ...
        errorMessageHint: 'Leave empty for the default message.',
        modelWhitelist: 'Model whitelist',
        modelWhitelistHint: 'Leave empty to apply to every model. Supports exact match and wildcard prefix (e.g., gpt-5.5*).',
      ...
      authSourceDefaults: {
        ...
        enabledHint: 'These defaults apply when a new user registers through this source. Grant on first bind applies only when an existing user binds this source.',
        sources: {
          email: {
            title: 'Email signup',
      ...
      paymentVisibleMethods: {
        ...
        sourceHint: 'Pick an explicit source before enabling the method. Methods that are not configured are not exposed.',
        sourceRequiredError: 'Pick a payment source before enabling {title}.'
      },
      ...
      user_error_view: {
        ...
        description: 'When enabled, users can see a redacted view of their failed requests on the usage page (no internal/upstream details). Requires ops monitoring to be enabled to have data.',
      },
      ...
      failedToLoad: 'Could not load settings',
      failedToSave: 'Could not save settings',
    ...
    errorPassthrough: {
      ...
      matchMode: {
        ...
        anyHint: 'Status code matches any error code, OR the message contains any keyword',
        allHint: 'Status code matches any error code, AND the message contains any keyword'
      },
      ...
      form: {
        ...
        platforms: 'Platforms',
        platformsHint: 'Leave empty to apply to every platform',
        ...
        customMessagePlaceholder: 'Error message to return to the client...',
        skipMonitoring: 'Skip monitoring',
        skipMonitoringHint: 'When enabled, errors matching this rule are not recorded in ops monitoring',
      ...
      deleteConfirm: 'Remove rule "{name}"?',
      failedToLoad: 'Could not load rules',
      failedToSave: 'Could not save the rule',
      failedToDelete: 'Could not delete the rule',
      failedToToggle: 'Could not toggle the status'
    },
    ...
    tlsFingerprintProfiles: {
      ...
      form: {
        ...
        pasteYamlHint: 'Paste the YAML copied from TLS Fingerprint Collector to auto-fill every field.',
        openCollector: 'Open Collector',
        parseYaml: 'Parse YAML',
        yamlParsed: 'YAML parsed successfully; fields auto-filled',
        yamlParseFailed: 'Could not parse YAML: name field not found',
        name: 'Profile Name',
      ...
      deleteConfirmMessage: 'Remove profile "{name}"? Accounts using this profile fall back to the built-in default.',
      createSuccess: 'Profile created successfully',
      updateSuccess: 'Profile updated successfully',
      deleteSuccess: 'Profile removed successfully',
