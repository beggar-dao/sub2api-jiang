export default {
    settings: {
      title: '系统设置',
      description: '管理注册、邮箱验证、默认值与 SMTP 配置',
      tabs: {
        general: '通用设置',
        agreement: '登录条款',
        features: '功能开关',
      ...
      features: {
        channelMonitor: {
          title: '渠道监控',
          description: '定期对已配置的渠道发起健康检查，向用户呈现可用性与延迟。关闭后调度器会停止扫描，用户端列表为空。',
          configureLink: '前往 渠道管理 > 渠道监控 配置监控项',
          enabled: '启用渠道监控',
          enabledHint: '关闭后后台不再执行定时检测，已有数据仍会保留。',
          defaultInterval: '默认检测间隔（秒）',
          defaultIntervalHint: '新建渠道监控时表单的默认值，可被单个渠道覆盖。范围 15 – 3600 秒。',
        },
        availableChannels: {
          title: '可用渠道',
          description: '向已登录用户展示其可访问的渠道、模型及定价聚合视图。默认关闭。',
        ...
        riskControl: {
          title: '风控中心',
          description: '启用内容审计菜单与全端点请求审核入口。默认关闭。',
          configureLink: '前往 风控中心 配置内容审计',
          enabled: '启用风控中心',
          enabledHint: '关闭后管理员侧边栏入口隐藏，网关内容审计不会执行。',
          cyberSessionBlock: 'cyber 会话自动屏蔽',
          cyberSessionBlockHint: '开启后，被上游网络安全策略(cyber_policy)拦截的会话将在 TTL 内被本地屏蔽，不再发往上游。仅屏蔽该会话，不影响同 Key 的其他会话。',
        ...
        affiliate: {
          ...
          freezeHoursDesc: '新产生的返利在冻结期内无法提现。0 = 不冻结。',
          durationDays: '返利有效期（天）',
          durationDaysDesc: '被邀请用户注册后多少天内的充值会产生返利。0 = 永久有效。',
          ...
          modal: {
            ...
            codeHint: '4-32 位，仅支持大写字母、数字、下划线、连字符；留空表示不修改；输入会自动转为大写。',
            rateLabel: '专属返利比例（可选）',
            ratePlaceholder: '例如 30',
            rateHint: '0-100%；留空（编辑模式下）表示清除专属比例并沿用全局。',
            errorBadRate: '请填写 0-100 之间的比例',
          ...
          batchModal: {
            ...
            placeholder: '例如 30',
            clearHint: '留空提交会清除所选用户的专属比例。',
          },
        },
      },
      emailTabDisabledTitle: '邮箱验证未启用',
      emailTabDisabledHint: '请先在「安全与认证」选项卡中启用邮箱验证，再配置 SMTP 设置。',
      registration: {
        title: '注册设置',
        description: '控制用户注册与验证',
        enableRegistration: '开放注册',
        enableRegistrationHint: '允许新用户注册',
        emailVerification: '邮箱验证',
        emailVerificationHint: '新用户注册时需验证邮箱',
        ...
        invitationCodeHint: '开启后，用户注册时需填写有效的邀请码',
        passwordReset: '忘记密码',
      ...
      apiKeyAcl: {
        ...
        description: '控制 API Key 白名单与黑名单使用哪个客户端 IP 进行判断',
      ...
      dingtalk: {
        ...
        bypassRegistrationHint: '即使「开放注册」关闭时也可通过钉钉登录来注册',
        syncDisplayName: '同步钉钉姓名',
        syncDisplayNameHint: '登录时把钉钉姓名写入 username 字段（同时记录到 dingtalk_name 属性）',
        syncCorpEmail: '同步企业邮箱',
        syncCorpEmailHint: '登录时把钉钉企业邮箱写入 dingtalk_email 属性（不影响登录邮箱）',
        syncCorpEmailPermissionHint: '需在钉钉开放平台 → 应用 → 权限管理中为本应用申请「邮箱等个人信息（fieldEmail）」权限，否则 OAPI 不会返回企业邮箱字段',
        syncDept: '同步部门',
        syncDeptHint: '登录时把钉钉首个部门完整路径写入 dingtalk_department 属性（每次登录实时拉取）',
      ...
      oidc: {
        ...
        frontendRedirectUrlHint: '后端回调完成后重定向至此前端路径',
      ...
      claudeCode: {
        ...
        description: '控制 Claude Code 客户端的访问要求',
        minVersion: '最低版本号',
      ...
      gatewayForwarding: {
        title: '请求转发行为',
        description: '控制请求转发至上游 OAuth 账号时的行为',
        ...
        cchSigning: 'CCH 签名',
        cchSigningHint: '对转发请求的 billing header 执行 CCH 哈希签名。关闭时保留原始占位符。',
        ...
        claudeOAuthSystemPromptBlocksHint: '每个 block 会保存为带 enabled、type、text、可选 cache_control 的 JSON。{billing_header} 会按请求动态生成；Claude Code 身份提示词与扩展提示词可直接编辑，也可用预设恢复默认值。',
        ...
        codexBlacklistDesc:
          '命中任一字段即拒绝，优先于一切放行规则。originator 精确匹配，User-Agent 为包含匹配（多个用逗号分隔）。',
        codexWhitelist: 'User-Agent/Originator 白名单',
        codexWhitelistDesc:
          '放行官方集之外的客户端：需 originator 精确，且每个 User-Agent 标记都命中。默认仍需通过引擎指纹门，勾「跳过引擎指纹」可免。',
      ...
      webSearchEmulation: {
        ...
        enabledHint: '全局开关。关闭后所有渠道与账号的 web search 模拟均不生效。',
        providers: '搜索服务商',
        addProvider: '添加服务商',
        providerType: '服务商类型',
        apiKey: 'API Key',
        apiKeyPlaceholder: '请填写 API Key',
      ...
      site: {
        ...
        backendModeDescription:
          '禁用用户注册、公开页面与自助服务功能。仅管理员可登录并管理平台。',
        siteName: '站点名称',
        siteNameHint: '显示在邮件与页面标题中',
        siteNamePlaceholder: 'Sub2API',
        siteSubtitle: '站点副标题',
        siteSubtitleHint: '显示在登录与注册页面',
        siteSubtitlePlaceholder: '订阅转 API 转换平台',
        apiBaseUrl: 'API 端点地址',
        apiBaseUrlHint: '用于"使用密钥"、"导入到 CC Switch"与回调地址建议，留空则使用当前站点地址',
        ...
        tablePageSizeOptionsFormatError: '可选每页条数格式无效，请填写 {min}-{max} 之间的整数并用英文逗号分隔',
        customEndpoints: {
          title: '自定义端点',
        ...
        contactInfoHint: '填写客服联系方式，会展示在兑换页面、个人资料等位置',
        docUrl: '文档链接',
        ...
        logoReadError: '读取图片文件未成功',
        homeContent: '首页内容',
        homeContentPlaceholder:
          '在此填写首页内容，支持 Markdown & HTML 代码。如果填写的是一个链接，会使用该链接作为 iframe 的 src 属性。',
        homeContentHint:
          '自定义首页内容，支持 Markdown/HTML。如果填写的是链接（以 http:// 或 https:// 开头），会使用该链接作为 iframe 的 src 属性，这允许你设置任意网页作为首页。设置后首页的状态信息将不再展示。',
        ...
        hideCcsImportButtonHint: '启用后会在 API Keys 页面隐藏"导入 CCS"按钮'
      },
      purchase: {
        title: '充值/订阅页面',
      ...
      soraClient: {
        ...
        enabled: '启用 Sora 客户端',
        enabledHint: '开启后，侧边栏会显示 Sora 入口，用户可访问 Sora 功能'
      },
      customMenu: {
        title: '自定义菜单页面',
        description: '添加自定义 iframe 页面到侧边栏导航。每个页面可设置为普通用户或管理员可见。',
        itemLabel: '菜单项 #{n}',
        name: '菜单名称',
      ...
      payment: {
        ...
        enabledPaymentTypesHint: '禁用服务商会同时禁用对应的实例。',
        ...
        alipayForceQRCodeHint: '启用后，移动端支付宝用户会统一使用二维码扫码支付，不再跳转至手机网站支付',
        helpText: '帮助文本',
        helpImageUrl: '帮助图片链接',
        ...
        noLimit: '留空表示不限制',
        helpImage: '帮助图片',
        helpImagePlaceholder: '上传或填写图片链接',
        helpTextPlaceholder: '请填写帮助说明文本...',
        providerEasypay: '易支付',
        ...
        validationEasyPayCustomMethodRequired: '每个易支付自定义方式都必须填写支付方式与上游 type',
        ...
        wxpayConfigHint: '微信支付通常只需填写 App ID。公众号 App ID、H5 应用名称、H5 应用地址仅在公众号支付或 H5 场景有特殊要求时再填写。',
        ...
        field_accountIdHint: '不涉及多账户、组织级密钥或连接账户收款时可不填；单账户 Scoped API Key 会默认使用所选账户。',
        field_cid: '支付渠道 ID',
        ...
        airwallexGuideSummary: '创建 Airwallex Scoped API 密钥时，建议只在账户级权限中为 Payment Acceptance 勾选读取与写入。',
        ...
        alipayGuideFaceToFaceFallback: '接口不可用或返回失败时，自动降级至电脑网站支付。',
        ...
        wxpayGuideNote: '当前表单默认共用一个 App ID，适合同主体下统一配置网页、移动与公众号场景。',
        wxpayGuideNativeTitle: 'Native / 扫码支付',
        wxpayGuideNativeOpen: '需开通 Native 或扫码支付能力。',
        wxpayGuideNativeCall: '桌面端默认调用 Native，下发二维码内容给前台渲染。',
        wxpayGuideNativeFallback: '移动端无法走 JSAPI 或 H5 时，也会自动回退至此。',
        wxpayGuideJsapiTitle: 'JSAPI / 公众号支付',
        wxpayGuideJsapiOpen: '需开通公众号支付，并确保当前浏览器在微信内且能拿到 OpenID。',
      ...
      balanceNotify: {
        ...
        enabled: '启用余额不足提醒',
        threshold: '默认提醒阈值',
        thresholdHint: '用户未自定义时采用此值',
        thresholdPlaceholder: '请填写金额',
        rechargeUrl: '充值页面 URL',
        rechargeUrlPlaceholder: 'https://example.com/payment',
        rechargeUrlHint: '设置后邮件中会包含充值链接按钮',
      },
      quotaNotify: {
        title: '账号限额通知',
      ...
      testEmail: {
        ...
        sending: '正在发送...',
        enterRecipientHint: '请填写收件人邮箱地址'
      },
      emailTemplates: {
        title: '邮件模板',
        description: '按事件与语言自定义通知邮件主题与 HTML 内容。',
        event: '事件',
        locale: '语言',
        localeEn: '英文',
        localeZh: '中文',
        subject: '主题',
        subjectPlaceholder: '请填写邮件主题',
        html: 'HTML 模板',
        ...
        restoring: '正在恢复...',
        restoreConfirm: '确定恢复此事件与语言的官方模板吗？当前自定义版本会被替换。',
        restoreSuccess: '已恢复官方模板',
        saveSuccess: '邮件模板已存储',
        placeholderCopied: '占位符已复制',
        validationRequired: '主题与 HTML 模板不能为空',
        empty: '当前没有可用的邮件模板事件或语言。',
      ...
      opsMonitoring: {
        ...
        realtimeEnabledHint: '启用实时请求速率与指标推送（WebSocket）',
        queryMode: '默认查询模式',
      ...
      adminApiKey: {
        ...
        create: '创建密钥',
        creating: '正在创建...',
        regenerateConfirm: '确定要重新生成吗？当前密钥会立即失效。',
        deleteConfirm: '确定要删除管理员 API Key 吗？外部集成会停止工作。',
      ...
      soraS3: {
        title: 'Sora 存储配置',
        description: '以多配置列表管理 Sora 媒体存储，支持 S3 与 Google Drive',
        newProfile: '新建配置',
        reloadProfiles: '刷新列表',
        empty: '当前没有存储配置，请先创建',
        createTitle: '新建存储配置',
        ...
        profileSaved: '存储配置存储成功',
        profileDeleted: '存储配置已删除',
        ...
        enabled: '启用存储',
        enabledHint: '启用后，Sora 生成的媒体文件会自动上传到存储',
        endpoint: 'S3 端点',
        region: '区域',
        bucket: '存储桶',
        prefix: '对象前缀',
        ...
        gdrive: {
          ...
          oauthFieldsRequired: '请先填写 Client ID 与 Client Secret',
          oauthSuccess: 'Google Drive 授权成功',
          oauthFailed: 'Google Drive 授权未成功',
          closeWindow: '此窗口将自动关闭',
          processing: '正在处理授权...',
          testStorage: '测试存储',
          testSuccess: 'Google Drive 存储测试通过（上传、访问、删除均正常）',
          testFailed: 'Google Drive 存储测试未成功'
        }
      },
      overloadCooldown: {
        title: '529 过载冷却',
      ...
      rateLimit429Cooldown: {
        ...
        saved: '429 默认回避设置存储成功',
        saveFailed: '存储 429 默认回避设置未成功'
      },
      streamTimeout: {
        title: '流超时处理',
      ...
      betaPolicy: {
        ...
        saved: 'Beta 策略设置存储成功',
        saveFailed: '存储 Beta 策略设置未成功',
        modelWhitelist: '模型白名单',
        modelWhitelistHint: '留空则对所有模型生效。支持精确匹配与通配符前缀（如 claude-opus-*）',
      ...
      openaiFastPolicy: {
        ...
        removeRule: '删除规则',
        addRule: '新增规则',
        saveHint: '存储时随系统设置一起提交（点击页面底部「存储」按钮）。',
      ...
      openaiExperimentalScheduler: {
        ...
        stickyWeightedDescription: '开启后 previous_response_id 与 session_hash 粘性进入高级调度打分；关闭时仍按旧逻辑硬命中粘性账号。',
        subscriptionPriorityTitle: '订阅优先',
        subscriptionPriorityDescription: '开启后先在 ChatGPT 订阅账号池中按权值选取；订阅池拿不到席位时再回退至非订阅账号池。',
        weightsTitle: '调度权值覆盖',
      ...
      saveSettings: '存储设置',
      saving: '正在存储...',
      settingsSaved: '设置存储成功',
      smtpConnectionSuccess: 'SMTP 连接成功',
      testEmailSent: '测试邮件发送成功',
      failedToLoad: '设置加载未成功',
      failedToSave: '存储设置未成功',
      failedToTestSmtp: 'SMTP 连接测试未成功',
      failedToSendTestEmail: '发送测试邮件未成功'
    },
    ...
    errorPassthrough: {
      ...
      failedToLoad: '规则加载未成功',
      failedToSave: '存储规则未成功',
      failedToDelete: '删除规则未成功',
      failedToToggle: '切换状态未成功'
    },
    ...
    tlsFingerprintProfiles: {
      ...
      form: {
        ...
        yamlParseFailed: 'YAML 解析未成功：未找到 name 字段',
        name: '模板名称',
      ...
      deleteConfirm: '删除模板',
      deleteConfirmMessage: '确定要删除模板 "{name}" 吗？使用此模板的账号会回退至内置默认值。',
      createSuccess: '模板创建成功',
      updateSuccess: '模板更新成功',
      deleteSuccess: '已删除模板',
      loadFailed: '模板加载未成功',
      saveFailed: '存储模板未成功',
      deleteFailed: '删除模板未成功'
    }
}
