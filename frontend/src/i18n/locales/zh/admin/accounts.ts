export default {
    accounts: {
      title: '账号管理',
      description: '管理 AI 平台账号与 Cookie',
      createAccount: '添加账号',
      autoRefresh: '自动刷新',
      ...
      listPendingSyncHint: '列表存在待同步变更，点击同步可补全最新数据。',
      ...
      syncFromCrsDesc:
        '将 claude-relay-service（CRS）中的账号同步至当前系统（不会在浏览器侧直接请求 CRS）。',
      ...
      syncNow: '开始同步',
      syncing: '正在同步...',
      syncMissingFields: '请填写服务地址、用户名与密码',
      syncResult: '同步结果',
      ...
      failedToToggleSchedulable: '切换调度状态未成功',
      ...
      schedulerScore: {
        ...
        stickyShort: '粘性',
        ungrouped: '未分组',
        hint: '展示格式为"分组名 / 基础分 / 粘性加分"。基础分按当前筛选条件限定的候选账号计算，包含优先级、负载、排队、错误率、首包延迟、重置窗口、额度余量等因子；粘性加分仅在开启粘性加权时用于 previous_response_id 或 session_hash。分数越大越优先。'
      },
      ...
      quotaNotify: {
        ...
        enabled: '启用告警',
        threshold: '告警金额',
        thresholdPlaceholder: '请填写百分比',
      },
      testConnection: '测试连接',
      reAuthorize: '重新授权',
      refreshToken: '刷新令牌',
      noAccountsYet: '当前没有账号',
      ...
      apiKey: 'API Key',
      deleteConfirm: "确定要删除账号 '{name}' 吗？该操作不可撤销。",
      failedToClearRateLimit: '清除速率限制未成功',
      platforms: {
        claude: 'Claude',
        openai: 'OpenAI',
        anthropic: 'Anthropic',
      ...
      tempUnschedulable: {
        ...
        rulesInvalid: '请至少填写一条包含错误码、关键词与时长的规则。',
        ...
        resetFailed: '恢复账号状态未成功',
        failedToLoad: '临时不可调度状态加载未成功',
      ...
      openaiQuotaReset: {
        ...
        confirmTitle: '确认重置周限',
        confirmMessage: '将消耗 1 次重置次数立即恢复当前窗口，剩余 {count} 次。该操作不可撤销，确定继续吗？'
      },
      tier: {
        free: 'Free',
        pro: 'Pro',
      ...
      form: {
        nameLabel: '账号名称',
        namePlaceholder: '请填写账号名称',
        platformLabel: '平台',
        selectPlatform: '选择平台',
        typeLabel: '类型',
        selectType: '选择类型',
        credentialsLabel: '凭证',
        credentialsPlaceholder: '请填写 Cookie 或 API Key',
        priorityLabel: '优先级',
      ...
      bulkEdit: {
        title: '批量编辑账号',
        selectionInfo: '已选择 {count} 个账号。只更新你勾选或填写的字段，未勾选的字段保持不变。',
        ...
        mixedPlatformWarning: '所选账号跨越多个平台（{platforms}）。展示的模型映射预设为合并结果——请确保映射对每个平台都适用。'
      },
      bulkDeleteTitle: '批量删除账号',
      bulkDeleteConfirm: '确定要删除选中的 {count} 个账号吗？该操作不可撤销。',
      bulkDeleteSuccess: '成功删除 {count} 个账号',
      bulkDeletePartial: '部分删除成功：成功 {success} 个，失败 {failed} 个',
      bulkDeleteFailed: '批量删除未成功',
      recoverState: '恢复状态',
      recoverStateHint: '用于恢复错误、限流与临时不可调度等可恢复状态。',
      recoverStateSuccess: '账号状态已恢复',
      recoverStateFailed: '恢复账号状态未成功',
      fallbackActive: '已回退',
      fallbackActiveTip: '原代理 {origin} 已到期，当前使用备用代理',
      revertProxy: '切回原代理',
      revertProxySuccess: '已成功切回原代理',
      revertProxyFailed: '切回原代理未成功',
      ...
      createSparkShadowFailed: '创建 Spark 影子账号未成功',
      resetStatus: '重置状态',
      statusReset: '账号状态已重置',
      failedToResetStatus: '重置账号状态未成功',
      cookieRefreshedSuccess: 'Cookie 刷新成功',
      testSuccess: '账号测试通过',
      testFailed: '账号测试未成功',
      failedToLoad: '账号列表加载未成功',
      failedToSave: '存储账号未成功',
      failedToDelete: '删除账号未成功',
      failedToRefresh: '刷新 Cookie 未成功',
      // Create/Edit Account Modal
      platform: '平台',
      accountName: '账号名称',
      enterAccountName: '请填写账号名称',
      accountType: '账号类型',
      ...
      vertexAnthropicHint: '使用 Google Cloud Service Account JSON 通过 Vertex AI 调用 Anthropic Claude。建议配置模型映射，将客户端 Claude 模型名映射至 Vertex 模型 ID。',
      vertexGeminiHint: '使用 Google Cloud Service Account JSON 访问 Vertex AI Gemini。建议将 Vertex 账号放入独立分组，避免与 AI Studio/Gemini OAuth 同模型混调。',
      ...
      vertexSaJsonKeyHidden: '密钥内容不会在表单中展示。',
      ...
      vertexSaJsonEditHint: 'Service Account JSON 不在编辑页展示；需要更换 JSON 时请删除账号后重新创建。',
      vertexProjectIdPlaceholder: '从 JSON 自动读取',
      vertexLocationHint: '不同 Vertex 模型可用的 location 可能不同，这里选择账号默认 endpoint location。',
      ...
      apiKeyHint: '你的 Claude Console API Key',
      // OpenAI specific hints
      openai: {
        baseUrlHint: '留空使用官方 OpenAI API',
        apiKeyHint: '你的 OpenAI API Key',
        oauthPassthrough: '自动透传（仅替换认证）',
        oauthPassthroughDesc:
          '开启后，该 OpenAI 账号会自动透传请求与响应，仅替换认证并保留计费/并发/审计及必要安全过滤；如遇兼容性问题可随时关闭回滚。',
      ...
      grok: {
        baseUrlHint: 'Grok OAuth 账号会转发至官方 xAI API Base URL。',
        apiKeyHint: 'Grok 订阅支持使用 OAuth refresh token；API Key 账号不在本次范围内。'
      },
      anthropic: {
        apiKeyPassthrough: '自动透传（仅替换认证）',
        apiKeyPassthroughDesc:
          '仅对 Anthropic API Key 生效。开启后，messages/count_tokens 请求会透传上游并仅替换认证，保留计费/并发/审计及必要安全过滤；关闭即可回滚到现有兼容链路。',
        apiKeyAuthScheme: '上游认证方式',
        apiKeyAuthSchemeDesc: '选择转发至 Anthropic-compatible 上游时使用的 API Key 认证头。Ollama Cloud 使用 Authorization: Bearer。',
      ...
      mapRequestModels: '将请求模型映射至实际模型。左边是请求的模型，右边是发送到 API 的实际模型。',
      ...
      syncUpstreamModelsLoading: '正在同步上游...',
      ...
      syncUpstreamModelsFailed: '同步上游模型未成功',
      syncUpstreamModelsError: '同步上游模型未成功：{message}',
      clearAllModels: '清除所有模型',
      customModelName: '自定义模型名称',
      enterCustomModelName: '请填写自定义模型名称',
      addModel: '填入',
      modelExists: '该模型已存在',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误会自动重试而不标记账号限流或错误，适用于上游指向另一个 sub2api 实例的场景。',
      ...
      enterErrorCode: '请填写错误码 (100-599)',
      invalidErrorCode: '请填写有效的 HTTP 错误码 (100-599)',
      errorCodeExists: '该错误码已被选中',
      interceptWarmupRequests: '拦截预热请求',
      interceptWarmupRequestsDesc: '启用后，标题生成等预热请求会返回 mock 响应，不消耗上游 token',
      headerOverride: {
        title: '请求头覆写',
        ...
        bulkDisableHint: '存储后会关闭所选账号的请求头覆写并清空已有配置。',
        bulkReplaceHint: '存储后会用下方配置整体替换所选账号已有的请求头覆写配置。',
        bulkEmptyRows: '请至少添加一行请求头再存储；如需清空已有配置，请关闭上方开关。',
        invalidName: '请求头名称格式不正确（仅允许字母、数字与 !#$%&\'*+-.^_`|~ 字符）',
      ...
      autoPauseOnExpiredDesc: '启用后，账号过期会自动暂停调度',
      ...
      mixedSchedulingTooltip:
        '！！注意！！ Antigravity Claude 与 Anthropic Claude 无法在同个上下文中使用，如果你同时有 Anthropic 账号与 Antigravity 账号，开启此选项会导致经常 400 报错。开启后，请用分组功能做好 Antigravity 账号与 Anthropic 账号的隔离。一定要弄明白再开启！！',
      ...
      failedToCreate: '创建账号未成功',
      failedToUpdate: '更新账号未成功',
      pleaseSelectStatus: '请选择有效的账号状态',
      mixedChannelWarningTitle: '混合渠道警告',
      mixedChannelWarning: '警告：分组 "{groupName}" 中同时包含 {currentPlatform} 与 {otherPlatform} 账号。混合使用不同渠道可能导致 thinking block 签名验证问题，会自动回退到非 thinking 模式。确定要继续吗？',
      pleaseEnterAccountName: '请填写账号名称',
      ...
      bedrockAccessKeyIdRequired: '请填写 AWS Access Key ID',
      bedrockSecretAccessKeyRequired: '请填写 AWS Secret Access Key',
      ...
      bedrockApiKeyRequired: '请填写 Bedrock API Key',
      ...
      upstream: {
        ...
        pleaseEnterBaseUrl: '请填写上游 Base URL',
        pleaseEnterApiKey: '请填写上游 API Key'
      },
      // OAuth flow
      oauth: {
        title: 'Claude 账号授权',
        ...
        authorizing: '正在授权...',
        followSteps: '请按以下步骤授权你的 Claude 账号：',
        step1GenerateUrl: '点击下方按钮生成授权 URL',
        generateAuthUrl: '生成授权 URL',
        generating: '正在生成...',
        regenerate: '重新生成',
        step2OpenUrl: '在浏览器中打开 URL 并完成授权',
        openUrlDesc: '在新标签页中打开授权 URL，登录你的 Claude 账号并授权。',
        proxyWarning: '注意：如果你配置了代理，请确保浏览器使用相同的代理访问授权页面。',
        step3EnterCode: '输入授权码',
        authCodeDesc: '授权完成后，页面会展示一个授权码。复制并粘贴到下方：',
        authCode: '授权码',
        authCodePlaceholder: '粘贴 Claude 页面的授权码...',
        authCodeHint: '粘贴从 Claude 页面复制的授权码',
        completeAuth: '完成授权',
        verifying: '正在验证...',
        pleaseEnterSessionKey: '请填写至少一个有效的 sessionKey',
        authFailed: '授权未成功',
        cookieAuthFailed: 'Cookie 授权未成功',
        ...
        openai: {
          title: 'OpenAI 账户授权',
          followSteps: '请按以下步骤完成 OpenAI 账户的授权：',
          step1GenerateUrl: '点击下方按钮生成授权链接',
          generateAuthUrl: '生成授权链接',
          step2OpenUrl: '在浏览器中打开链接并完成授权',
          openUrlDesc: '请在新标签页中打开授权链接，登录你的 OpenAI 账户并授权。',
          importantNotice:
          ...
          authCodeHint: '你可以直接复制整个链接或仅复制 code 参数值，系统会自动识别',
          failedToGenerateUrl: '生成 OpenAI 授权链接未成功',
          failedToExchangeCode: 'OpenAI 授权码兑换未成功',
          failedToValidateRT: '验证 Refresh Token 未成功',
          errors: {
            OPENAI_OAUTH_PROXY_REQUIRED:
              '未设置代理，当前服务器无法直连 OpenAI，导致 OpenAI OAuth 请求未成功。请先选择可访问 OpenAI 的代理后重试；如果授权码已失效，请重新生成授权链接。'
          },
          // Refresh Token auth
          refreshTokenAuth: '手动输入 RT',
          refreshTokenDesc: '填写你已有的 OpenAI Refresh Token，支持批量输入（每行一个），系统会自动验证并创建账号。',
          refreshTokenPlaceholder: '粘贴你的 OpenAI Refresh Token...\n支持多个，每行一个',
          ...
          codexSessionEmpty: '请填写 Codex JSON 或 accessToken',
          codexSessionImportFailed: 'Codex 账号导入未成功',
          ...
          codexPatDesc: '填写 Codex at- Personal Access Token，系统会先调用 OpenAI whoami 校验后再创建账号。',
          ...
          codexPatEmpty: '请填写 Codex Personal Access Token',
          codexPatImportFailed: 'Codex PAT 账号创建未成功',
          sessionTokenAuth: '手动输入 ST',
          sessionTokenDesc: '填写你已有的 Session Token，支持批量输入（每行一个），系统会自动验证并创建账号。',
          sessionTokenPlaceholder: '粘贴你的 Session Token...\n支持多个，每行一个',
          sessionTokenRawLabel: '原始字符串',
          sessionTokenRawPlaceholder: '粘贴 /api/auth/session 原始数据或 Session Token...',
          sessionTokenRawHint: '支持粘贴完整 JSON，系统会自动解析 ST 与 AT。',
          ...
          pleaseEnterSessionToken: '请填写 Session Token'
        },
        grok: {
          title: 'Grok 账号授权',
          followSteps: '请按以下步骤授权你的 xAI/Grok 账号：',
          ...
          refreshTokenAuth: '手动输入 RT',
          refreshTokenDesc: '填写已有的 xAI refresh token，支持批量输入（每行一个）。',
          refreshTokenPlaceholder: '粘贴你的 xAI refresh token...\n支持多个，每行一个',
          validating: '正在验证...',
          validateAndCreate: '验证并创建账号',
          pleaseEnterRefreshToken: '请填写 Refresh Token',
          failedToGenerateUrl: '生成 Grok 授权链接未成功',
          missingExchangeParams: '缺少授权码、state 或 OAuth 会话',
          failedToExchangeCode: 'Grok 授权码兑换未成功',
          failedToValidateRT: '验证 Grok refresh token 未成功',
          errors: {
        ...
        gemini: {
          title: 'Gemini 账户授权',
          followSteps: '请按以下步骤完成 Gemini 账户的授权：',
          ...
          step2OpenUrl: '在浏览器中打开链接并完成授权',
          openUrlDesc: '请在新标签页中打开授权链接，登录你的 Google 账户并授权。',
          ...
          failedToGenerateUrl: '生成 Gemini 授权链接未成功',
          missingExchangeParams: '缺少 code / session_id / state',
          failedToExchangeCode: 'Gemini 授权码兑换未成功',
          missingProjectId:
            'GCP Project ID 获取未成功：你的 Google 账号未关联有效的 GCP 项目。请前往 Google Cloud Console 激活 GCP 并绑定信用卡，或在授权时手动填写 Project ID。',
          modelPassthrough: 'Gemini 直接转发模型',
          modelPassthroughDesc: '所有模型请求将直接转发至 Gemini API，不进行模型限制或映射。',
          stateWarningTitle: '提示',
          stateWarningDesc: '建议粘贴完整回调链接（包含 code 与 state）。',
        ...
        antigravity: {
          ...
          followSteps: '请按以下步骤完成 Antigravity 账户的授权：',
          step1GenerateUrl: '生成授权链接',
          generateAuthUrl: '生成授权链接',
          step2OpenUrl: '在浏览器中打开链接并完成授权',
          openUrlDesc: '请在新标签页中打开授权链接，登录你的 Google 账户并授权。',
          ...
          authCodePlaceholder:
            '方式1：复制完整的链接\n(http://localhost:xxx/auth/callback?code=...)\n方式2：仅复制 code 参数的值',
          authCodeHint: '你可以直接复制整个链接或仅复制 code 参数值，系统会自动识别',
          failedToGenerateUrl: '生成 Antigravity 授权链接未成功',
          missingExchangeParams: '缺少 code / session_id / state',
          failedToExchangeCode: 'Antigravity 授权码兑换未成功',
          // Refresh Token auth
          refreshTokenAuth: '手动输入 RT',
          refreshTokenDesc: '填写你已有的 Antigravity Refresh Token，支持批量输入（每行一个），系统会自动验证并创建账号。',
          refreshTokenPlaceholder: '粘贴你的 Antigravity Refresh Token...\n支持多个，每行一个',
          validating: '正在验证...',
          validateAndCreate: '验证并创建账号',
          pleaseEnterRefreshToken: '请填写 Refresh Token',
          failedToValidateRT: '验证 Refresh Token 未成功'
      ...
      gemini: {
        ...
        apiKeyHint: '你的 Gemini API Key（以 AIza 开头）',
        tier: {
          label: '账号等级',
      ...
      sendingImageRequest: '正在发送生图测试请求...',
      response: '响应：',
      startTest: '开始测试',
      retry: '重试',
      copyOutput: '复制输出',
